...
//   const fetchData = () => {
//     return axios
//       .get(`http://localhost:8080/api/users/${saved.id}/products`)
//       .then((data) => {
//         data.map((dat) => {
...
useEffect(() => {
    ...
    fetch(`http://localhost:8080/api/users/${saved.id}/products`)
      .then((response) => response.json())
      .then((data) => {
        // data.map((dat)=>{
        //   if(dat.title.length>20){
  ...
  return (
    <div>
      <header className={styles.header}>
        ...
        {saved != null ? <LogoutDisplay isfavorites={true}></LogoutDisplay> : null}
      </header>
      <div></div>
      {/* <div>{searchWord}</div>
      <div>{category}</div> */}
      <section className={styles.content}>
        {content ? (
          <Products isfavorites={true} currency={currency} products={data} />
        ) : (
          <Spinner />
        )}
      </section>
